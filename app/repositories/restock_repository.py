...
class RestockRepository:
    ...
    def update_stock(self, vehicle: Vehicle, current_quantity: int) -> None:
        ...
        vehicle.quantity = current_quantity  # type: ignore[assignment]
        self.db.add(vehicle)

    def create_restock_history(
        self, vehicle_id: int, admin_id: int, quantity: int
    ) -> RestockHistory:
        """Log a history record of the restock event."""
        history = RestockHistory(
            vehicle_id=vehicle_id, admin_id=admin_id, quantity=quantity
        )
        self.db.add(history)
        return history
