...
def test_access_with_invalid_token(client):
    response = client.get(
        "/api/purchase/history", headers={"Authorization": "Bearer invalid.token.here"}
    )
    assert response.status_code == 401
...
def test_expired_token(client):
    ...
    expired_token = create_access_token(
        data={"sub": "1", "role": "USER"}, expires_delta=timedelta(seconds=-1)
    )

    response = client.get(
        "/api/purchase/history", headers={"Authorization": f"Bearer {expired_token}"}
    )
    assert response.status_code == 401
...
def test_wrong_authentication_scheme(client):
    response = client.get(
        "/api/purchase/history", headers={"Authorization": "Basic somerandombase64"}
    )
    assert response.status_code == 401
...
def test_missing_bearer_prefix(client):
    ...
    response = client.get("/api/purchase/history", headers={"Authorization": token})
    assert response.status_code == 401
