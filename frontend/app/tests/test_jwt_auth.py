...
def test_access_with_invalid_token(client):
    response = client.get(
        "/api/vehicles", headers={"Authorization": "Bearer invalid.token.here"}
    )
    assert response.status_code == 401
...
def test_expired_token(client):
    ...
    response = client.get(
        "/api/vehicles", headers={"Authorization": f"Bearer {expired_token}"}
    )
    assert response.status_code == 401
...
def test_wrong_authentication_scheme(client):
    response = client.get(
        "/api/vehicles", headers={"Authorization": "Basic somerandombase64"}
    )
    assert response.status_code == 401
...
def test_missing_bearer_prefix(client):
    ...
    response = client.get("/api/vehicles", headers={"Authorization": token})
    assert response.status_code == 401
