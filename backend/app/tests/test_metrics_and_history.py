...
def setup_metrics_data(client: TestClient):
    ...
    # Create users
    admin = User(email="admin@test.com", hashed_password=get_password_hash("pass"), role=Role.ADMIN)
    user = User(email="user@test.com", hashed_password=get_password_hash("pass"), role=Role.USER)
    db.add(admin)
    db.add(user)
    db.commit()

    # Create vehicles
    v1 = Vehicle(make="Honda", model="Civic", category="Sedan", price=20000.0, quantity=5)
    v2 = Vehicle(make="Ford", model="Mustang", category="Sports", price=40000.0, quantity=0)
    db.add(v1)
    db.add(v2)
    db.commit()

    # Create purchases
    p1 = PurchaseHistory(user_id=user.id, vehicle_id=v1.id, quantity=1, total_price=20000.0)
    db.add(p1)
    db.commit()

    admin_token = client.post("/api/auth/login", json={"email": "admin@test.com", "password": "pass"}).json()["access_token"]
    user_token = client.post("/api/auth/login", json={"email": "user@test.com", "password": "pass"}).json()["access_token"]
...
def test_admin_metrics(client: TestClient, setup_metrics_data):
    ...
    response = client.get("/api/metrics/admin", headers={"Authorization": f"Bearer {token}"})
