...
async def seed_db():
    ...
    vehicles = [
        Vehicle(
            make="Toyota", model="Camry XSE", category="Sedan", price=32000.00, quantity=5,
        ...
        Vehicle(
            make="Ford", model="Mustang GT", category="Sports", price=45000.00, quantity=2,
        ...
        Vehicle(
            make="Tesla", model="Model Y", category="SUV", price=52000.00, quantity=10,
        ...
        Vehicle(
            make="Honda", model="Civic Type R", category="Sports", price=43000.00, quantity=0,
    ...
    for v_data in vehicles:
        existing = db.query(Vehicle).filter(Vehicle.make == v_data.make, Vehicle.model == v_data.model).first()
        if not existing:
            db.add(v_data)
