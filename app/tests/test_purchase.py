...
def setup_data(client: TestClient):
    ...
    v1 = {"make": "Ford", "model": "Mustang", "category": "Coupe", "price": 50000.0, "quantity": 10}
    ...
    v2 = {"make": "Chevy", "model": "Camaro", "category": "Coupe", "price": 40000.0, "quantity": 0}
...
def test_purchase_vehicle_successfully(client: TestClient, setup_data):
    ...
    response = client.post(f"/api/vehicles/{v1_id}/purchase", json={"quantity": 2}, headers=headers)
...
def test_vehicle_not_found(client: TestClient, setup_data):
    headers = get_auth_headers(client, is_admin=False)
    response = client.post("/api/vehicles/9999/purchase", json={"quantity": 1}, headers=headers)
    assert response.status_code == 404

def test_out_of_stock(client: TestClient, setup_data):
    headers = get_auth_headers(client, is_admin=False)
    v2_id = setup_data["v2_id"]
    response = client.post(f"/api/vehicles/{v2_id}/purchase", json={"quantity": 1}, headers=headers)
    assert response.status_code == 400
    assert "stock" in response.json()["detail"].lower() or "quantity" in response.json()["detail"].lower()

def test_purchase_more_than_stock(client: TestClient, setup_data):
    headers = get_auth_headers(client, is_admin=False)
    v1_id = setup_data["v1_id"]
    response = client.post(f"/api/vehicles/{v1_id}/purchase", json={"quantity": 15}, headers=headers)
    assert response.status_code == 400
    assert "stock" in response.json()["detail"].lower() or "quantity" in response.json()["detail"].lower()
...
def test_purchase_quantity_zero(client: TestClient, setup_data):
    headers = get_auth_headers(client, is_admin=False)
    v1_id = setup_data["v1_id"]
    response = client.post(f"/api/vehicles/{v1_id}/purchase", json={"quantity": 0}, headers=headers)
    assert response.status_code == 422 # Pydantic validation error

def test_purchase_negative_quantity(client: TestClient, setup_data):
    headers = get_auth_headers(client, is_admin=False)
    v1_id = setup_data["v1_id"]
    response = client.post(f"/api/vehicles/{v1_id}/purchase", json={"quantity": -5}, headers=headers)
    assert response.status_code == 422 # Pydantic validation error
...
def test_invalid_jwt(client: TestClient, setup_data):
    v1_id = setup_data["v1_id"]
    response = client.post(f"/api/vehicles/{v1_id}/purchase", json={"quantity": 1}, headers={"Authorization": "Bearer invalidtoken"})
    assert response.status_code == 401

def test_purchase_history_created(client: TestClient, setup_data):
    headers = get_auth_headers(client, is_admin=False)
    v1_id = setup_data["v1_id"]
    response = client.post(f"/api/vehicles/{v1_id}/purchase", json={"quantity": 1}, headers=headers)
    assert response.status_code == 200
...
def test_remaining_quantity_updated(client: TestClient, setup_data):
    ...
    client.post(f"/api/vehicles/{v1_id}/purchase", json={"quantity": 3}, headers=headers)
...
def test_transaction_rollback(client: TestClient, setup_data):
    ...
    headers = get_auth_headers(client, is_admin=False)
    v1_id = setup_data["v1_id"]
    client.post(f"/api/vehicles/{v1_id}/purchase", json={"quantity": 100}, headers=headers)
