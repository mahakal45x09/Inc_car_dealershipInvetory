...
class PurchaseService:
    ...
    def purchase_vehicle(self, user_id: int, vehicle_id: int, request: PurchaseRequest) -> PurchaseResponse:
        """
        ...
        try:
            ...
            if not vehicle:
                logger.warning(f"Vehicle {vehicle_id} not found during purchase attempt by user {user_id}")
            ...
            total_price = vehicle.price * request.quantity
            remaining_quantity = vehicle.quantity - request.quantity

            # Persist changes
            self.repository.update_vehicle_stock(vehicle, remaining_quantity)
            self.repository.create_purchase(user_id, vehicle_id, request.quantity, total_price)
            self.repository.commit()

            logger.info(f"User {user_id} successfully purchased {request.quantity}x of vehicle {vehicle_id}. Total: ${total_price}")
            ...
            self.repository.rollback()
            logger.error(f"Unexpected error during purchase for user {user_id} on vehicle {vehicle_id}: {str(e)}")
            raise HTTPException(
