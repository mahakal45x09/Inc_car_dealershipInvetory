...
class RestockService:
    ...
    def restock_vehicle(
        self, admin_id: int, vehicle_id: int, request: RestockRequest
    ) -> RestockResponse:
        """
        ...
        try:
            ...
            if not vehicle:
                logger.warning(
                    f"Vehicle {vehicle_id} not found during restock attempt by admin {admin_id}"
                )
                raise VehicleNotFoundException()

            if request.quantity <= 0:
                logger.warning(
                    f"Admin {admin_id} attempted to restock vehicle {vehicle_id} with invalid quantity: {request.quantity}"
                )
                raise HTTPException(
            ...
            current_quantity = int(vehicle.quantity) + request.quantity

            # Atomic save
            self.repository.update_stock(vehicle, int(current_quantity))
            self.repository.create_restock_history(
                vehicle_id, admin_id, request.quantity
            )
            self.repository.commit()

            logger.info(
                f"Admin {admin_id} successfully restocked vehicle {vehicle_id} by {request.quantity} units. Total stock is now {current_quantity}."
            )
            ...
            self.repository.rollback()
            logger.error(
                f"Unexpected error during restock by admin {admin_id} on vehicle {vehicle_id}: {str(e)}"
            )
            raise HTTPException(
